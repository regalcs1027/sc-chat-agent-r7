...
**⚠️ テーブルを追加・変更したら `SCHEMA_VERSION` を上げる**

`create_tables()` は `@st.cache_resource` でプロセスに1回しか実行されない。
Streamlit Cloud は再デプロイ時にプロセスを作り直さず、スクリプトを再実行するだけなので、
バージョンを上げないと新しいテーブルが作られず `UndefinedTable` で落ちる
（`app_settings` の追加時に実際に踏んだ）。`app.py` の `SCHEMA_VERSION` を +1 すれば、
キャッシュキーが変わって初期化が再実行される。Reboot でも直るが、上げるのが正しい。
